;(function(){
  ...
  Customizlet._lines = { info:[], tip:[], warn:[] };
  Customizlet._console = ('console' in window && 'log' in console && 'warn' in console && 'info' in console);
  ...
  function dmlink(str, url){
    return '<a style="'+Customizlet.reset+'text-decoration:underline;color:#844" href="'+url+'">'+html(str)+'</a>';
  }
  ...
  Customizlet.saveResults = function() {
    var hResults = {
      warn: Customizlet._lines.warn, tip: Customizlet._lines.tip, info: Customizlet._lines.info, statsHTML: Customizlet.statsHTML
    }, summary = "", prognosis = $("Customizlet_results_prognosis");

    if (prognosis) summary = prognosis.innerHTML + ", ";

    summary += document.getElementsByTagName('*').length + ' elements';
    summary = summary.replace("'", "\\'");
    Customizlet.SaveToJdrop("DOM Monster", hResults, Customizlet.Version, summary);
  };

  Customizlet.SaveToJdrop = function(appname, myDataObj, version, summary) {
  ...
  Customizlet.JdropCallback = function(jsonobj) {
    if (!document.getElementById("Customizlet_results_prognosis_container")) {
      setTimeout(function() { Customizlet.JdropCallback(jsonobj); }, 500);
      return;
    }
    Customizlet._lines.warn = jsonobj['warn'];
    Customizlet._lines.tip = jsonobj['tip'];
    Customizlet._lines.info = jsonobj['info'];
    Customizlet.statsHTML = jsonobj['statsHTML'];
    Customizlet.flush();
  };

  window.__DOMMonsterJdropCallBack = Customizlet.JdropCallback;

  Customizlet.close = function(){
    var results = $('Customizlet_results_tips');
    results.parentNode.removeChild(results);
  };

  Customizlet.flush = function(string){
    var results = $('Customizlet_results_tips'),
      html = '<div style="'+Customizlet.reset+';margin-left:230px;padding-top:4px">';
    function flushArray(array){
      for(var i=0;i<array.length;i++)
        html += '<div style="'+Customizlet.reset+'margin:0 0 4px 0;padding:4px 0px 0px 0px;font-size:11px">' + array[i] + '</div>';
    }
    var prognosis = $("Customizlet_results_prognosis"),
      container = $("Customizlet_results_prognosis_container"),
      warnings = Customizlet._lines.warn.length;
    if(warnings>0) {
      if(warnings>2) {
        container.style.cssText += ';color:#A02523';
        prognosis.innerHTML = 'whoa, potentially huge issues';
        $('Customizlet_results_warnings').innerHTML = Customizlet._lines.warn.length + ' warnings indicate app ill-health';
      } else {
        container.style.cssText += ';color:#E8871D';
        prognosis.innerHTML = 'room for improvement';
        $('Customizlet_results_warnings').innerHTML = Customizlet._lines.warn.length + ' warning' + (warnings==1?'':'s');
      }
      $('Customizlet_results_warnings_container').style.cssText += ';display:inline';
    } else {
      container.style.cssText += ';color:#40a40F';
      prognosis.innerHTML = 'yay! you\'re doing a great job!';
      $('Customizlet_results_warnings_container').style.cssText += ';display:none';
    }

    flushArray(Customizlet._lines.warn);
    flushArray(Customizlet._lines.tip);
    flushArray(Customizlet._lines.info);
    html += '</div>';
    results.innerHTML += html;

    $('Customizlet_stats').innerHTML = Customizlet.statsHTML;
  };

  Customizlet.log = function(string, hint, type){
    type = type || 'tip';
    hint = hint || '';
    var color = { info: '888', tip:'88f', warn:'efb000' }[type];
    Customizlet._lines[type].push(
      '<div style="'+Customizlet.reset+'text-transform:uppercase;font-size:10px;border:1px solid #'+color+';width:32px;color:#'+color+';-webkit-border-radius:5px;padding:1px;float:left;text-align:center;margin:-2px 4px 0px 0px">'+type+'</div> '+
      '<strong style="'+Customizlet.reset+'font-weight:bold">'+string+'</strong> '+hint);
  };
  Customizlet.tip = function(string, hint){ Customizlet.log(string,hint,'tip'); };
  Customizlet.info = function(string, hint){ Customizlet.log(string,hint,'info'); };
  Customizlet.warn = function(string, hint){ Customizlet.log(string,hint,'warn'); };

  Customizlet.trace = function(msgs){
    var formatted = "";
    for(var i=0;i<msgs.length;i++)
      formatted += html(msgs[i]) + '<br>';

    Customizlet._lines['info'].push(
      '<div style="'+Customizlet.reset+';font-family:monospace;border:1px solid #888;padding:5px">'+formatted+'</div>'
    );
  }

  Customizlet.time = function(scope){
    Customizlet.time.scope = Customizlet.time.scope || {};
    if(Customizlet.time.scope[scope]) {
      var duration = (new Date()).getTime()-Customizlet.time.scope[scope];
      Customizlet.time.scope[scope] = null;
      return duration/1000;
    } else {
      Customizlet.time.scope[scope] = (new Date()).getTime();
      return null;
    }
  };

  Customizlet.benchmark = function(method, times, scope){
    var i = times || 1000;
    Customizlet.time(scope||'benchmark');
    while(i--) method();
    return Customizlet.time(scope||'benchmark')/times;
  };

  Customizlet.scriptTagsTips = function(){
    ...
    if(count>2 && count<6)
      Customizlet.tip('Found '+count+' &lt;script&gt; tags on page.','Try to reduce the number of script tags.');
    if(count.length>=6)
      Customizlet.warn('Found '+count+' &lt;script&gt; tags on page.','Try to reduce the number of script tags.');
  ...
  // inline scripts block rendering
    ...
    if(longInlines.length>0)
      Customizlet.tip('Found ' + longInlines.length + ' big (>=2kB) inline script'+((longInlines.length>1)?'s':'')+'.', 'Try to avoid big inline scripts, they block rendering and won\'t get cached.');

    Customizlet.noLoaders = function() {
      var loaders = ['head', 'yepnope', '$LAB', 'jsl', 'JSLoader'], r = true;
      for (var i=0;i<loaders.length;i++)
        if (loaders[i] in window) {
          Customizlet.tip("You're using "+loaders[i]+", a JavaScript loader library.",
  ...
  Customizlet.versionCompare = function(userVersion, edgeVersion) {
  ...
  Customizlet.frameworkTips = function(){
    // Version number on http://prototypejs.org/download
    if('Prototype' in window && Customizlet.versionCompare(Prototype.Version, [1, 7]))
      Customizlet.tip("You are using the Prototype JavaScript framework v"+Prototype.Version+".","There's a newer version available, which potentially includes performance updates.");

    // Version number on http://script.aculo.us/downloads
    if('Scriptaculous' in window && Customizlet.versionCompare(Scriptaculous.Version, [1, 9, 0]))
      Customizlet.tip("You are using script.aculo.us v"+Scriptaculous.Version+".","There's a newer version available, which potentially includes performance updates.");

    // Version number on http://jquery.com/
    if(typeof jQuery == 'function'){
      if(Customizlet.versionCompare(jQuery.prototype.jquery, [1, 5, 2])) {
        Customizlet.tip("You are using the jQuery JavaScript framework v"+jQuery.prototype.jquery+".","There's a newer version available, which potentially includes performance updates.");
      }
      // Version number on http://jqueryui.com/home
      if(jQuery.ui && Customizlet.versionCompare(jQuery.ui.version, [1, 8, 11])) {
        Customizlet.tip("You are using the jQuery UI JavaScript framework v"+jQuery.ui.version+".","There's a newer version available, which potentially includes performance updates.");
      }
    }

    // Version number on http://download.dojotoolkit.org/
    if(typeof dojo == 'object' && Customizlet.versionCompare(dojo.version.toString(), [1, 5, 0]) && !(dojo.version.toString().match(/dev/)))
      Customizlet.tip("You are using the dojo JavaScript toolkit v"+dojo.version.toString()+".","There's a newer version available, which potentially includes performance updates.");

    // Version number on http://developer.yahoo.com/yui/
    if(typeof YAHOO == 'object' && typeof YAHOO.evn == 'object' && Customizlet.versionCompare(YAHOO.env.getVersion('yahoo').version, [2, 8, 2]))
      Customizlet.tip("You are using the Yahoo! User Interface Library 2 v"+YAHOO.env.getVersion('yahoo').version+".","There's a newer version available, which potentially includes performance updates.");

    // Version number on http://developer.yahoo.com/yui/3/
    if('YUI' in window && typeof YUI == 'function' && Customizlet.versionCompare(YUI().version, [3, 3, 0]))
      Customizlet.tip("You are using the Yahoo! User Interface Library 3 v"+YUI().version+".","There's a newer version available, which potentially includes performance updates.");

    // Version number on http://mootools.net/download
    if(typeof MooTools == 'object' && (!MooTools.version || Customizlet.versionCompare(MooTools.version, [1, 3])))
      Customizlet.tip("You are using the MooTools JavaScript tools v"+MooTools.version+".","There's a newer version available, which potentially includes performance updates.");

    // Version number Extjs on http://www.sencha.com/products/js/download.php
    if(typeof Ext === 'object' && Customizlet.versionCompare(Ext.version, [3, 3, 1]))
      Customizlet.tip("You are using the Ext JS v"+Ext.version+".","There's a newer version available, which potentially includes performance updates.");

    // Version number on http://rightjs.org/
    if('RightJS' in window && Customizlet.versionCompare(RightJS.version, [2, 2, 3]))
      Customizlet.tip("You are using the RightJS JavaScript framework v"+RightJS.version+".","There's a newer version available, which potentially includes performance updates.");
  };

  Customizlet.webfontTips = function(){
    ...
    if(typeof Typekit == 'object')
      Customizlet.tip("You are using the Typekit webfont service.", tiptext);
    ...
    while(i--) {
      ...
      if(isFontService(href)){
        if(Customizlet._console) console.log(href);
        Customizlet.tip("You are using an external webfont service.", tiptext);
        return "";
      }
      if(j==0) continue;
      while(j--){
  ...
  Customizlet.iFrameTips = function(){
    var nodes = $tagname('iframe');
    if(nodes.length>0 && nodes.length<4)
      Customizlet.tip('Reduce the number of &lt;iframe&gt; tags.','There are '+nodes.length+' iframe elements on the page.');
    if(nodes.length>=4)
      Customizlet.warn('Reduce the number of &lt;iframe&gt; tags','There are '+nodes.length+' iframe elements on the page.');
  };

  Customizlet.cssTips = function(){
    ...
    if(nodes.length>1 && nodes.length<8)
      Customizlet.tip('Reduce the number of &lt;link rel="stylesheet"&gt; tags.','There are '+nodes.length+' external stylesheets loaded on the page.');
    if(nodes.length>=8)
      Customizlet.warn('Reduce the number of &lt;link rel="stylesheet"&gt; tags','There are '+nodes.length+' external stylesheets loaded on the page.');
    }
    function styleAttributeTips(){
      var nodes = $tagname('*'), i = nodes.length, styleNodes = 0, styleBytes = 0;
      while(i--)
        if(nodes[i].style.cssText.length > 0){
          if(Customizlet._console) console.warn('Inline style', nodes[i]);
          styleNodes++;
          styleBytes += nodes[i].style.cssText.length + 8;
        }
      if(styleNodes>0)
        Customizlet.tip('Reduce the number of tags that use the style attribute, replacing it with external CSS definitions.', styleNodes+' nodes use the style attribute, for a total of '+styleBytes+' bytes.');
    }
    function dontAtImport() {
      ...
      if (present)
        Customizlet.tip('Using @import in a style element will impact rendering performance.', 'Use the &lt;link&gt; tag instead. See '+dmlink('this article', 'http://www.stevesouders.com/blog/2009/04/09/dont-use-import/')+' for details.');
    }
    function checkForShadows() {
      ...
      if (shadowCount > 0)
        Customizlet.tip('Using the box-shadow property can introduce serious scroll & resize lag in the browser.', 'Consider replacing with border-image or reducing the number of elements with shadows (currently: ' + shadowCount + ')');
    }
    linkTagTips();
    styleAttributeTips();
    dontAtImport();
  ...
  Customizlet.doctypeTips = function(){
   ...
   Customizlet.flashTips = function() {
     ...
     if (nodes.length == 1) {
       Customizlet.tip('Consider alternatives to using Flash.', 'There is 1 Flash object embedded. Replacing this with browser-native implementations (SVG, VML, Canvas) could lead to better loading times, especially if the Flash plugin is loaded first.');
     } else if (nodes.length) {
       Customizlet.tip('Consider alternatives to using Flash.', 'There are ' + nodes.length + ' Flash objects embedded. Replacing these with browser-native implementations (SVG, VML, Canvas) could lead to better loading times, especially if the Flash plugin is loaded first.');
     }
   };

  Customizlet.getStyle = function(element, style) {
  ...
  Customizlet.opacityTips = function(){
    var nodes = $tagname('*'), op = [], i = nodes.length;
    while(i--){
      var opacity = Customizlet.getStyle(nodes[i],'opacity') || 1;
      if(opacity<1) {
        nodes[i].style.cssText += ';border:1px dashed #00f';
        op.push(nodes[i]);
        if(Customizlet._console) console.info('Transparent node', nodes[i]);
      }
    }
    if(op.length>0 && op.length < 10)
      Customizlet.tip('Some nodes use transparency.','To improve rendering performance, try not to use the CSS opacity property (found '+op.length+' nodes, marked with a dashed blue border).');
    if(op.length >= 10)
      Customizlet.warn('Lots of nodes use transparency.','To improve rendering performance, try not to use the CSS opacity property (found '+op.length+' nodes, marked with a dashed blue border).');
  };

  Customizlet.nodesTips = function(){
    ...
    while(i--) {
      ...
      if (nodes[i].childNodes.length==0 && !(tag=='link' || tag=='br' || tag=='script' || tag=='meta' || tag=='img' ||
        ...
        if(Customizlet._console) console.warn('Empty node', nodes[i]);
        empty++;
      }

      if(DEPRECATED.indexOf(tag)>-1) {
        if(Customizlet._console) console.warn('Deprecated node', nodes[i]);
      ...
      if(tag=='link' && /stylesheet|icon|shortcut|prefetch/.test(nodes[i].rel) && nodes[i].getAttribute('href') === ''){
        if(Customizlet._console) console.warn('Empty href attribute', nodes[i]);
        emptyAttr++;
      }
      ...
      if(tag=='html'){
        ...
        if(attribute && attribute.value === ''){
          if(Customizlet._console) console.warn('Empty manifest attribute', nodes[i]);
          emptyAttr++;
        }
      }
      ...
      if(tag=='video' || tag=='audio' || tag=='iframe' || tag=='input' || tag=='embed' || tag == 'img'){
        ...
        if(attribute && attribute.value === '' ){
          if(Customizlet._console) console.warn('Empty src attribute', nodes[i]);
          emptyAttr++;
        }
      }
      ...
      while( j-- ){
        ...
        if(attribute){
            if(Customizlet._console) console.warn('Inline JavaScript', nodes[i]);
    ...
    function findWhitespaceTextnodes(element){
      ...
      if(element.nodeType==3 && /^\s+$/.test(element.nodeValue)){
        // if(Customizlet._console) console.warn('Whitespace-only text node', element);
        whitespace++;
      }
    ...
    Customizlet.stats(nodecount, 'nodes', level(nodecount,1500,3000));
    Customizlet.stats(textnodes, 'text nodes', level(textnodes,750,1500));
    Customizlet.stats((textnodeLength/1024).toFixed(1)+'k', 'text node size', level(textnodeLength,80000,500000));
    Customizlet.stats(contentPercent.toFixed(2)+'%', 'content percentage', revlevel(contentPercent, 25, 50));

    if(empty) Customizlet.tip('There are '+empty+' empty nodes.','Removing them might improve performance.');
    if(deprecated) {
      var tags = [];
      for(tag in deprecatedTags) tags.push(tag.toUpperCase());
      Customizlet.tip('There are '+deprecated+' nodes which use a deprecated tag name ('+tags.join(', ')+').','Try updating this content to HTML5.');
    }
    if(multiIds.length > 0){
      Customizlet.warn('There '+((multiIds.length==1)?'is ':'are ')+multiIds.length+' duplicate id'+((multiIds.length>1)?'s':'')+' for nodes in your document.', 'Node ids must be unique within the HTML document. See JavaScript console for details.');
      if(Customizlet._console) console.warn('Duplicate element ids found', unique(multiIds));
      if(Customizlet._console) console.warn('Nodes affected by duplicate ids', multiIdsElements);
    }
    if(whitespace)
      Customizlet.tip(((whitespace/nodecount)*100).toFixed(1)+'% of nodes are whitespace-only text nodes.','Reducing the amount of whitespace, like line breaks and tab stops, can help improve the loading and DOM API performance of the page.');
    if(comments)
      Customizlet.tip('There are '+comments+' HTML comments.','Removing the comments can help improve the loading and DOM API performance of the page.');
    if(emptyAttr)
      Customizlet.warn('There are '+emptyAttr+' HTML elements with empty source attributes.', 'Removing these nodes or updating the attributes will prevent double-loading of the page in some browsers. See this article for more information: '+dmlink('Empty image src can destroy your site','http://www.nczonline.net/blog/2009/11/30/empty-image-src-can-destroy-your-site/'))
    if(js&&js_byte)
      Customizlet.tip('There are '+js_byte+' bytes of inline JavaScript code in '+js+' HTML nodes.', 'Removing the inline JavaScript, or updating the attributes will improve the loading speed of the page.');
  };

  Customizlet.statsHTML = '';
  Customizlet.stats = function(value, stat, type){
    var color = { low: '80E41F', mid: 'E8871D', high: 'A02523' };
    Customizlet.statsHTML +=
      '<div style="'+Customizlet.reset+'margin:0;margin-left:5px;padding:0;margin-bottom:4px;height:auto"><div style="'+Customizlet.reset+';float:left;width:13px;height:13px;margin-right:2px;background:#'+color[type||'low']+'"> </div>'+
      '<strong>'+value+'</strong> '+stat+
      '</div>'
  };

  Customizlet.globals = function(){
    ...
    if(found.length > 5){
      Customizlet.tip('Found '+found.length+' JavaScript globals.','Cutting back on globals can increase JavaScript performance.' + (Customizlet._console ? ' See JavaScript console for details.' : ''));
      if(Customizlet._console) console.log('Found more than 5 globals on your page.', found);
    }
  };

  Customizlet.performanceTips = function(){
    ...
    Customizlet.stats(nodes.length, 'elements', level(nodes.length,750,1500));

    Customizlet.nodesTips();
    Customizlet.stats(average.toFixed(1), 'average nesting depth', level(average,8,15));
    Customizlet.stats((domsize/1024).toFixed(1)+'k', 'serialized DOM size', level(domsize,100*1024,250*1024));

    if(domsize>(100*1024) && domsize<(250*1024))
      Customizlet.tip('Your serialized DOM size is a little high.','Performance might improve if you reduce the amount of HTML.');
    if(domsize>=(250*1024))
      Customizlet.warn('DOM size is higher than 250k.','Performance might improve if you reduce the amount of HTML.');

    var bodycount = Customizlet.benchmark(function(){
      document.body.appendChild(document.createTextNode(' '));
      var x = document.body.innerHTML;
    }, 10);
    Customizlet.stats(bodycount.toFixed(3)+'s', 'serialization time', level(bodycount,0.05,0.1));
    if(bodycount>0.1)
      Customizlet.warn('Average DOM serialization speed is '+bodycount.toFixed(3)+'s.','Try to reduce the complexity of the DOM structure.');

    if(nodes.length>1500)
      Customizlet.warn('Element count seems excessively high.','Performance might improve if you reduce the number of nodes.');
    if(average>8 && average<=15)
      Customizlet.tip('Nesting depth is a little high.','Reducing it might increase performance.');
    if(very)
      Customizlet.warn('Nesting depth is very high.','Some of the nodes are nested more than 15 levels deep (these are marked with a dashed red border).');

    Customizlet.cssTips();
    Customizlet.doctypeTips();
    Customizlet.frameworkTips();
    Customizlet.webfontTips();
    Customizlet.scriptTagsTips();
    Customizlet.iFrameTips();
    Customizlet.opacityTips();
    Customizlet.flashTips();
    Customizlet.globals();

    if(Customizlet._lines.tip.length == 0 && Customizlet._lines.warn.length == 0)
      Customizlet.tip('No tips! Congratulations! It seems your site is up to speed!');
  };

  var IE = !!(window.attachEvent && navigator.userAgent.indexOf('Opera') === -1);
  if(IE) Customizlet.getStyle = function(element, style) {
  ...
  var old = $('Customizlet_results_tips');
  if(old) old.parentNode.removeChild(old);
  setTimeout(function(){
    var error;

    if(Customizlet._console)
      Customizlet.info('Check the JavaScript console of your browser for detailed information on some of the tips.');
    try {
      if("undefined" === typeof(JDROPVIEW)) Customizlet.performanceTips();
    } catch(e) {
      error = e;
    }
    if(error){
      Customizlet.info('Error while analyzing page. '+
    ...
    Customizlet.flush();
    setTimeout(function(){
      $('Customizlet_results_tips').style.cssText += ';-webkit-transform:translateY(0px)';
    }, 10);

    $('Customizlet_results_tips').saveResults = Customizlet.saveResults;
  },10);
 })();
